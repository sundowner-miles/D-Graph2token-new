...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.weight.dtype in [torch.float16, torch.bfloat16, torch.float32]:
...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        ...
        if self.config.rope_scaling is None:
            ...
            # 新增：检查 rope_scaling 字典是否包含必要的 "type" 和 "factor" 键
            if not isinstance(self.config.rope_scaling, dict) or "type" not in self.config.rope_scaling or "factor" not in self.config.rope_scaling:
                # 配置不完整，降级到基础 RoPE 初始化（兼容 Llama-3 默认配置）
                print(f"警告：rope_scaling 配置不完整（缺少 type/factor 键），降级到基础 RoPE 初始化")
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _update_causal_mask(self, attention_mask, input_tensor):
        ...
        if hasattr(self, "causal_mask"):  # we use the current dtype to avoid any overflows
            # 优化点：只根据当前序列长度 seq_length 进行切片，避免分配 max_position_embeddings 大小的巨型矩阵
            causal_mask = (
                self.causal_mask[None, None, :seq_length, :seq_length].repeat(batch_size, 1, 1, 1).to(dtype) * torch.finfo(dtype).min
            )
        else:
            mask = torch.full(
